...
use crate::constant::{self, SELECT_FUN_RESULT_JSON, WORK_DIR};

use super::{Extractor, Func};
...
impl CodeQLExtractor {
    ...
    */
    fn exec_ql(&self, ql: &Path, res: &Path) -> Result<Output, std::io::Error> {
        ...
        let _opt = cmd
            ...
            .arg(
                ql.to_str()
                    .expect(format!("The path of ql {:?} is not valid", ql).as_str()),
            )
            .args([
                "-o",
                res.to_str()
                    .expect(format!("The path of result {:?} is not valid", res).as_str()),
            ])
            .args([
                "-d",
                self.database_pathbuf.to_str().expect(
                    format!("The database path {:?} is not valid", self.database_pathbuf).as_str(),
                ),
            ])
            .output();
    ...
    */
    fn exec_select_ql(&self) -> PathBuf {
        let select_func_ql = PathBuf::new()
        ...
        if let Ok(o) = opt {
            log::debug!(
                "Execute ql file stdout: {}",
                String::from_utf8(o.stdout).unwrap()
            );
            log::debug!(
                "Execute ql file stderr: {}",
    ...
    fn convert_bqrs2json(bqrs_path: &Path, json_path: &Path) {
        ...
        let _opt = cmd
            ...
            .arg(
                bqrs_path
                    .to_str()
                    .expect("The path of result database is not valid"),
            )
            .args([
                "-o",
                json_path
                    .to_str()
                    .expect("The path of result json is not valid"),
            ])
